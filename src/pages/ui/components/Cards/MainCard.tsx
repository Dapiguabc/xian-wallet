import { Box, Button, Card, CardBody, CardHeader, Flex, Heading, IconButton, Tab, TabIndicator, TabList, TabPanel, TabPanels, Tabs, Text, useClipboard, useToast } from '@chakra-ui/react';
...
const TokenCard: React.FC<TokenMeta & Props> = ({name, amount, symbol, next, back }) => {
    return (
        <Box border="1px solid #dee2e6" borderRadius='5px' p='10px'>
            <Box mt="5px" mb="15px" fontWeight='bold'>{name} ({symbol})</Box>
            <Box mt="5px" mb="15px">{amount}</Box>
            <Flex my="5px" gap='10px'>
                <Button flexBasis='50%' leftIcon={<FaPaperPlane />}  onClick={() => next(4)}>Send</Button>
                <Button flexBasis="50%" leftIcon={<FaDownload />}  onClick={back}>Receive</Button>
            </Flex>
        </Box>
    );
}
...
const HistoryCard: React.FC<HistoryMeta> = (props) => {
    ...
    return (
        <Box border="1px solid #dee2e6" borderRadius='5px' p='10px'>
            <Box textOverflow='ellipsis' whiteSpace='nowrap' overflow='hidden'>{hash}</Box>
            <Box>{contract}</Box>
            <Box>{method}</Box>
            <Box>{status}</Box>
    ...
    <Card {...cardStyle}>
      ...
      <CardBody p={0} mb="10px" width='100%' lineHeight={1.5}>
        ...
        <Box mb='20px'>
            ...
            <Text onClick={clipboard} cursor='pointer' color="blue.500" _hover={{textDecoration:'underline'}} textOverflow='ellipsis' whiteSpace='nowrap' overflow='hidden'>{currentAccount}</Text>
        </Box>
            ...
            <TabPanels py="20px">
                <TabPanel p={0}>
                    <Box mb='20px' textAlign='center'>
                        <Text display='inline-block' textAlign='center' fontSize='xl' fontWeight='bold'>Tokens</Text>
                        <IconButton
                            onClick={() => next(6)}
                            float='right'
                            variant='solid'
                    ...
                    {
                        tokenList.map(item => <TokenCard key={item.contract} {...item} next={next} back={back} />)
                    }
                </TabPanel>
                <TabPanel p={0}>
                    <Box mb='20px' textAlign='center'>
                        <Text display='inline-block' textAlign='center' fontSize='xl' fontWeight='bold'>Local Activity</Text>
                        <IconButton
